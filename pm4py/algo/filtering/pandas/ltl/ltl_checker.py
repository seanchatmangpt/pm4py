...
def eventually_follows(
    ...
    enable_timestamp = exec_utils.get_param_value(
        Parameters.ENABLE_TIMESTAMP,
        parameters,
        len(timestamp_diff_boundaries) > 0,
    )
    ...
    df_a = [
        df[df[attribute_key] == attribute_value].copy()
    ...
    for i in range(2, len(df_a)):
        df_join = df_join.merge(
            df_a[i], on=case_id_glue, suffixes=("", "_%d" % i)
        ).dropna()
        df_join["@@diffindex%d" % (i - 1)] = (
            df_join[constants.DEFAULT_INDEX_KEY + "_%d" % i]
            - df_join[constants.DEFAULT_INDEX_KEY + "_%d" % (i - 1)]
        )
        ...
        for i in range(1, len(df_a)):
            df_join["@@difftimestamp%d" % (i - 1)] = (
                pandas_utils.get_total_seconds(
                    df_join[timestamp_key + "_%d" % i]
                    - df_join[timestamp_key + "_%d" % (i - 1)]
                )
            )
            ...
            if timestamp_diff_boundaries:
                df_join = df_join[
                    df_join["@@difftimestamp%d" % (i - 1)]
                    >= timestamp_diff_boundaries[i - 1][0]
                ]
                df_join = df_join[
                    df_join["@@difftimestamp%d" % (i - 1)]
                    <= timestamp_diff_boundaries[i - 1][1]
                ]
...
def A_next_B_next_C(
    ...
    df_join = (
        df_A.join(df_B, on=case_id_glue, rsuffix="_2")
        .dropna()
        .join(df_C, on=case_id_glue, rsuffix="_3")
        .dropna()
    )
    df_join["@@diffindex"] = (
        df_join[constants.DEFAULT_INDEX_KEY + "_2"]
        - df_join[constants.DEFAULT_INDEX_KEY]
    )
    df_join["@@diffindex2"] = (
        df_join[constants.DEFAULT_INDEX_KEY + "_3"]
        - df_join[constants.DEFAULT_INDEX_KEY + "_2"]
    )
...
def four_eyes_principle(
    ...
    df_join_pos = df_join[
        df_join[resource_key] == df_join[resource_key + "_2"]
    ]
    df_join_neg = df_join[
        df_join[resource_key] != df_join[resource_key + "_2"]
    ]
