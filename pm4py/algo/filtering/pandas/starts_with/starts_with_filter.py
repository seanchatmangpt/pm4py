...
from pm4py.statistics.traces.generic.pandas.case_statistics import (
    get_variants_df,
)
...
def apply(
    ...
    if isinstance(first_case_variant, tuple):
        ...
        variants_df["variant"] = variants_df["variant"].apply(
            lambda x: constants.DEFAULT_VARIANT_SEP.join(list(x))
        )
        admitted_prefixes = [
            constants.DEFAULT_VARIANT_SEP.join(x) for x in admitted_prefixes
        ]
    admitted_prefixes = tuple(admitted_prefixes)

    variants_df = variants_df[
        variants_df["variant"].str.startswith(admitted_prefixes)
    ]
