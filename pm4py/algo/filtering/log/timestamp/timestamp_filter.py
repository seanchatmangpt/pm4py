...
def filter_on_trace_attribute(
    ...
    trace_attribute = exec_utils.get_param_value(
...
def is_contained(trace, dt1, dt2, timestamp_key):
    ...
    if trace:
        if (
            strpfromiso.fix_naivety(trace[0][timestamp_key]) >= dt1
            and strpfromiso.fix_naivety(trace[-1][timestamp_key]) <= dt2
        ):
            return True
    return False
...
def filter_traces_contained(
    ...
    filtered_log = EventLog(
        [
            trace
            for trace in log
            if is_contained(trace, dt1, dt2, timestamp_key)
        ],
        attributes=log.attributes,
        extensions=log.extensions,
        omni_present=log.omni_present,
...
def is_intersecting(trace, dt1, dt2, timestamp_key):
    ...
    if trace:
        condition1 = (
            dt1 <= strpfromiso.fix_naivety(trace[0][timestamp_key]) <= dt2
        )
        condition2 = (
            dt1 <= strpfromiso.fix_naivety(trace[-1][timestamp_key]) <= dt2
        )
        condition3 = (
            strpfromiso.fix_naivety(trace[0][timestamp_key])
            <= dt1
            <= strpfromiso.fix_naivety(trace[-1][timestamp_key])
        )
        condition4 = (
            strpfromiso.fix_naivety(trace[0][timestamp_key])
            <= dt2
...
def filter_traces_intersecting(
    ...
    timestamp_key = (
        ...
        if PARAMETER_CONSTANT_TIMESTAMP_KEY in parameters
...
def apply_events(
    ...
    filtered_stream = EventStream(
        [
            x
            for x in stream
            if dt1 <= strpfromiso.fix_naivety(x[timestamp_key]) <= dt2
        ],
        attributes=log.attributes,
        extensions=log.extensions,
        omni_present=log.omni_present,
...
def has_attribute_in_timeframe(
    trace, attribute, attribute_value, dt1, dt2, timestamp_key
):
    for e in trace:
        if (
            attribute in e
            and e[attribute] == attribute_value
            and dt1 <= strpfromiso.fix_naivety(e[timestamp_key]) <= dt2
        ):
            return True
    return False
