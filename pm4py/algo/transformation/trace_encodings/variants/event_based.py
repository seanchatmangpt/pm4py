...
def apply(
    ...
    """
    Encodes each event in each trace as a feature vector.

    From an event-log point of view, the result keeps the trace/event grid:
    each trace becomes a sequence of event vectors, padded to the maximum trace
    length in the log. String event attributes are one-hot encoded; numeric
    event attributes are copied as numeric values.

    Example with event attribute "concept:name":
        case 1: A, B
        case 2: A

    The vocabulary is [event:concept:name@A, event:concept:name@B]. If the
    maximum trace length is 2, the encoded tensor is:
        case 1 -> [[1, 0], [0, 1]]
        case 2 -> [[1, 0], [0, 0]]
