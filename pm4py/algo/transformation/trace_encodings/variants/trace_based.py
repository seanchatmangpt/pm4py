...
def apply(
    ...
    """
    Encodes each trace as one feature vector.

    From an event-log point of view, each case is summarized into a single row.
    String trace attributes and selected string event attributes become one-hot
    columns; selected numeric trace attributes and the last seen selected
    numeric event attributes become numeric columns. Optional extra features
    can add durations, activity positions, path times, concurrency, and
    workload measures.

    Example with event attribute "concept:name":
        case 1: A, B, A
        case 2: A, C

    The activity columns are [event:concept:name@A, event:concept:name@B,
    event:concept:name@C]. The encoded rows are:
        case 1 -> [1, 1, 0]
        case 2 -> [1, 0, 1]

    This differs from count2vec because this legacy trace-based representation
    stores whether a selected string attribute value appears in the trace,
    ...
    parameters
        ...
        - NUMERIC_ATTRIBUTE_AGGREGATIONS => collection of numeric aggregations to compute: last, first, min, max,
        mean, median, stdev, and sum
