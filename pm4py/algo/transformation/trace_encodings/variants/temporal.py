...
def apply(
    ...
    """
    Aggregates an event log into temporal windows and computes window-level
    process features.

    From an event-log point of view, events are grouped by a timestamp
    frequency such as weekly ("W") or daily ("D"). Each output row describes
    one time window, not one trace. The columns include counts and averages
    such as unique resources, unique activities, number of events, average
    arrival rate, average finish rate, waiting time, service time, rework
    count, cases per resource, events per case, number of cases, and resources
    per case.

    Example with GROUPER_FREQ="D":
        day 2024-01-01 has 2 cases and 5 events
        day 2024-01-02 has 1 case and 2 events

    The result has two rows, one for each day, with columns such as
    number_of_cases=[2, 1] and num_events=[5, 2].
