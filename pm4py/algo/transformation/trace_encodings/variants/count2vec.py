"""
Trace-level count2vec encoding.
...
row per trace and one column per token; the cell is the number of times the
token appears in that trace.
...
If event_attributes includes additional columns, each event token can include
context such as activity, resource, and amount before counts are computed.
...
The survey lists count2vec as a baseline encoding and uses sklearn as the
open-source implementation source. This variant uses scikit-learn's
CountVectorizer over trace tokens.
"""
...
def apply(
    ...
    """
    Encodes each trace as token frequency counts.
    ...
    -------
    data
        One row per trace, one count per discovered token.
    feature_names
        Token names corresponding to the columns of data.
    """
    return sklearn_vectorization.apply(
        log, binary=False, ngram_range=(1, 1), parameters=parameters
    )
