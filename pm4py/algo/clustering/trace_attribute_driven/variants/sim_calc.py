...
def dist_calc(
    var_list_1, var_list_2, log1, log2, freq_thres, num, alpha, parameters=None
):
    """
    ...
    if var_list_1 == var_list_2:
        ...
        for i in range(max_len):
            ...
            df_1_suc = suc_dist_calc.occu_var_suc(
                max_var[i], parameters={"binarize": True}
            )
            for j in range(min_len):
                df_2_act = act_dist_calc.occu_var_act(min_var[j])
                df_2_suc = suc_dist_calc.occu_var_suc(
                    min_var[j], parameters={"binarize": True}
                )

                df_act = pandas_utils.merge(
                    df_1_act, df_2_act, how="outer", on="var"
                ).fillna(0)
                df_suc = pandas_utils.merge(
                    df_1_suc, df_2_suc, how="outer", on="direct_suc"
                ).fillna(0)

                dist_vec_act[j] = pdist(
                    np.array(
                        [df_act["freq_x"].values, df_act["freq_y"].values]
                    ),
                    "cosine",
                )[0]
                dist_vec_suc[j] = pdist(
                    np.array(
                        [df_suc["freq_x"].values, df_suc["freq_y"].values]
                    ),
                    "cosine",
                )[0]

                if single:
                    if (abs(dist_vec_act[j]) <= 1e-8) and (
                        abs(dist_vec_suc[j]) <= 1e-6
                    ):  # ensure both are 1
                        max_freq_act[i] = (
                        ...
                        max_loc_col_act = np.argmin(
                            dist_vec_act
                        )  # location of max value
                        max_loc_col_suc = np.argmin(
                            dist_vec_suc
                        )  # location of max value
                        max_freq_act[i] = (
                            var_count_max.iloc[i]
                            * var_count_min.iloc[max_loc_col_act]
                        )
                        max_freq_suc[i] = (
                            var_count_max.iloc[i]
                            * var_count_min.iloc[max_loc_col_suc]
                        )
                        max_per_var_act[i] = (
                            dist_vec_act[max_loc_col_act] * max_freq_act[i]
                        )
                        max_per_var_suc[i] = (
                            dist_vec_suc[max_loc_col_suc] * max_freq_suc[i]
                        )
    ...
    if single:
        ...
        dist = (
            np.sum(col_sum_act) * alpha + np.sum(col_sum_suc) * (1 - alpha)
        ) / vec_sum
