...
def add_xor_gateway(bpmn, counts):
    ...
    split = BPMN.ExclusiveGateway(
        name="", gateway_direction=BPMN.Gateway.Direction.DIVERGING
    )
    join = BPMN.ExclusiveGateway(
        name="", gateway_direction=BPMN.Gateway.Direction.CONVERGING
    )
    bpmn.add_node(split)
    bpmn.add_node(join)
...
def add_inclusive_gateway(bpmn, counts):
    ...
    split = BPMN.InclusiveGateway(
        name="", gateway_direction=BPMN.Gateway.Direction.DIVERGING
    )
    join = BPMN.InclusiveGateway(
        name="", gateway_direction=BPMN.Gateway.Direction.CONVERGING
    )
    bpmn.add_node(split)
    bpmn.add_node(join)
...
def recursively_add_tree(
    parent_tree, tree, bpmn, initial_event, final_event, counts, rec_depth
):
    from pm4py.objects.bpmn.obj import BPMN
    ...
    if tree.operator is None:
        ...
        for subtree in tree_childs:
            bpmn, counts, x, y = recursively_add_tree(
                tree,
                subtree,
                bpmn,
                split_gateway,
        ...
        for i in range(len(tree_childs)):
            bpmn, counts, initial_connect, final_connect = (
        ...
        bpmn, counts, i, y = recursively_add_tree(
            tree, do, bpmn, join, split, counts, rec_depth + 1
        )
        for redo in tree_childs[1:]:
            bpmn, counts, x, y = recursively_add_tree(
                tree, redo, bpmn, split, join, counts, rec_depth + 1
            )
...
def apply(tree, parameters=None):
    ...
    bpmn, counts, _, _ = recursively_add_tree(
        tree, tree, bpmn, start_event, end_event, counts, 0
    )
