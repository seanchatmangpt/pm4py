...
def apply(
    ...
    """
    ev_types_list = pandas_utils.format_unique(
        ocel.events[ocel.event_activity].unique()
    )
    obj_types_list = pandas_utils.format_unique(
    ...
    overall_o2o[ocel.object_type_column] = overall_o2o[
        ocel.object_id_column
    ].map(obj_type_map)
    overall_o2o[ocel.object_type_column + "_2"] = overall_o2o[
        ocel.object_id_column + "_2"
    ].map(obj_type_map)

    o2o_list = list(
        overall_o2o.groupby(
            [ocel.object_type_column, ocel.object_type_column + "_2"]
        )
        .size()
        .to_dict()
    )
    changes_list = pandas_utils.format_unique(
        ocel.object_changes[ocel.object_type_column].unique()
    )
    ...
    for e2ot in e2o_list:
        e2o = ocel.relations[
            (ocel.relations[ocel.event_activity] == e2ot[0])
            & (ocel.relations[ocel.object_type_column] == e2ot[1])
        ]
    ...
    for o2ot in o2o_list:
        o2o = overall_o2o[
            (overall_o2o[ocel.object_type_column] == o2ot[0])
            & (overall_o2o[ocel.object_type_column + "_2"] == o2ot[1])
        ]
        o2o = o2o.dropna(axis="columns", how="all")
        dct_types_rel["o2o"][o2ot] = o2o

    for objt in changes_list:
        objects = ocel.object_changes[
            ocel.object_changes[ocel.object_type_column] == objt
        ]
        objects = objects.dropna(axis="columns", how="all")
