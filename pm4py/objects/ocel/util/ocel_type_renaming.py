...
def __rename_types_from_maps(
    ocel: OCEL,
    event_types_map: Optional[Dict[str, str]],
    object_types_map: Optional[Dict[str, str]],
) -> OCEL:
    ret_ocel = deepcopy(ocel)

    if event_types_map is not None:
        ret_ocel.events[ocel.event_activity] = ret_ocel.events[
            ocel.event_activity
        ].map(event_types_map)
        ret_ocel.relations[ocel.event_activity] = ret_ocel.relations[
            ocel.event_activity
        ].map(event_types_map)

    if object_types_map is not None:
        ret_ocel.objects[ocel.object_type_column] = ret_ocel.objects[
            ocel.object_type_column
        ].map(object_types_map)
        ret_ocel.relations[ocel.object_type_column] = ret_ocel.relations[
            ocel.object_type_column
        ].map(object_types_map)
        ret_ocel.object_changes[ocel.object_type_column] = (
            ret_ocel.object_changes[ocel.object_type_column].map(
                object_types_map
            )
        )
...
def remove_spaces_non_alphanumeric_characters_from_types(ocel: OCEL) -> OCEL:
    ...
    """
    object_types = (
        ocel.objects[ocel.object_type_column].value_counts().to_dict()
    )
...
def abbreviate_event_types(ocel: OCEL) -> OCEL:
    ...
    """
    event_types = list(
        ocel.events[ocel.event_activity].value_counts().to_dict()
    )
