...
def apply(
    ...
    concatener = exec_utils.get_param_value(
        Parameters.CONCATENER, parameters, "@@"
    )
    concat_column = exec_utils.get_param_value(
        Parameters.CONCAT_COLUMN, parameters, "@@concat"
    )

    ocel = copy(ocel)
    ocel.relations[concat_column] = (
        ocel.relations[ocel.event_id_column]
        + concatener
        + ocel.relations[ocel.object_id_column]
    )
    relations = ocel.relations.groupby(ocel.object_id_column)[
    ...
    if qualifier_type == "creation":
        ...
        relations = {
            y + concatener + x: qualifier_type for x, y in relations.items()
        }
    ...
    ocel.relations[ocel.qualifier + "_2"] = ocel.relations[concat_column].map(
        relations
    )
    ocel.relations[ocel.qualifier + "_2"] = ocel.relations[
        ocel.qualifier + "_2"
    ].fillna(ocel.relations[ocel.qualifier])
