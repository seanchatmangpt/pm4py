...
def apply(ocel: OCEL, param: str, parameters: Optional[Dict[Any, Any]] = None):
    ...
    ev_param = new_ocel.events.dropna(subset=[param])[
        [event_id, event_activity, event_timestamp, param]
    ]
    ...
    new_objects_df = pandas_utils.instantiate_dataframe(
        [{object_id: v, object_type: param} for v in vals]
    )
    ...
    new_ocel.relations = pandas_utils.insert_index(
        new_ocel.relations,
        internal_index,
        reset_index=False,
    ...
    new_ocel.relations = new_ocel.relations.sort_values(
