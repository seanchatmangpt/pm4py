...
def apply(
    ocel: OCEL, file_path: str, parameters: Optional[Dict[Any, Any]] = None
):
    if parameters is None:
        parameters = {}

    enable_names_stripping = exec_utils.get_param_value(
        Parameters.ENABLE_NAMES_STRIPPING, parameters, True
    )
    ...
    event_types = sorted(
        pandas_utils.format_unique(EVENTS["ocel_type"].unique())
    )
    object_types = sorted(
        pandas_utils.format_unique(OBJECTS["ocel_type"].unique())
    )
    ...
    if enable_names_stripping:
        EVENT_CORR_TYPE["ocel_type_map"] = EVENT_CORR_TYPE[
            "ocel_type_map"
        ].apply(lambda x: names_stripping.apply(x))
        OBJECT_CORR_TYPE["ocel_type_map"] = OBJECT_CORR_TYPE[
            "ocel_type_map"
        ].apply(lambda x: names_stripping.apply(x))
    ...
    for act in e_types:
        df = ocel.events[ocel.events[event_activity] == act].dropna(
    ...
    for ot in o_types:
        df = ocel.objects[ocel.objects[object_type] == ot].dropna(
            how="all", axis="columns"
        )
        df = df.rename(columns={object_id: "ocel_id"})
        del df[object_type]

        df2 = ocel.object_changes[
            ocel.object_changes[object_type] == ot
