...
class ObjMarkingAccessor(object):
    ...
    def __sub__(self, other):
        if isinstance(other, int):
            picked_items = set(
                random.sample(
                    self.obj_marking.__getitem_original__(self.key),
                    min(
                        len(self.obj_marking.__getitem_original__(self.key)),
                        other,
                    ),
                )
            )
            self.obj_marking.picked_items = picked_items
            diff = self.obj_marking.__getitem_original__(self.key).difference(
                picked_items
            )
            return diff
        else:
    ...
    def __add__(self, other):
        if isinstance(other, int):
            picked_items = set(
                random.sample(self.obj_marking.picked_items, other)
            )
            if self.key not in self.obj_marking:
                self.obj_marking[self.key] = set()
            new_value = self.obj_marking.__getitem_original__(self.key).union(
                picked_items
            )
