...
class HeuristicsNet:
    def __init__(
        ...
        self.node_type = (
            "frequency" if self.performance_dfg is None else "performance"
        )
        ...
        if self.activities_occurrences is None:
            ...
            for act in self.activities:
                self.activities_occurrences[act] = (
                    dfg_utils.sum_activities_count(frequency_dfg, [act])
                )
            ...
            if node_name in other_net.nodes:
                ...
                for out_node1 in node1.output_connections:
                    if out_node1.node_name in n2n:
                        node1.output_connections[out_node1] = (
                            node1.output_connections[out_node1]
                            + node2.output_connections[
                                n2n[out_node1.node_name]
                            ]
                        )
                for out_node2 in node2.output_connections:
                    if out_node2.node_name not in n1n:
                        if out_node2.node_name in copied_self.nodes:
                            ...
                            node1.output_connections[nn] = (
                                node2.output_connections[out_node2]
                            )
                        else:
                            node1.output_connections[out_node2] = (
                                node2.output_connections[out_node2]
                            )
        diffext = [
            other_net.nodes[node]
            for node in other_net.nodes
            if node not in copied_self.nodes
        ]
        for node in diffext:
            copied_self.nodes[node.node_name] = node
        copied_self.start_activities = (
            copied_self.start_activities + other_net.start_activities
        )
        copied_self.end_activities = (
            copied_self.end_activities + other_net.end_activities
        )
        copied_self.default_edges_color = (
            copied_self.default_edges_color + other_net.default_edges_color
        )
