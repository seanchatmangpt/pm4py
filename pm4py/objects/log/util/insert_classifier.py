def search_act_class_attr(log, force_activity_transition_insertion=False):
    ...
    if (
        log.classifiers
        and "Activity classifier" in log.classifiers
        and log.classifiers["Activity classifier"]
    ):
        classifier = "Activity classifier"
    elif (
        log.classifiers
        and "MXML Legacy Classifier" in log.classifiers
        and log.classifiers["MXML Legacy Classifier"]
    ):
        classifier = "MXML Legacy Classifier"
    return insert_activity_classifier_attribute(
        log,
        classifier,
...
def insert_activity_classifier_attribute(
    log, classifier, force_activity_transition_insertion=False
):
    """
    ...
    if classifier is not None:
        ...
        for trace in log:
            for event in trace:
                classifier_value = "+".join(
                    [event[x] for x in log.classifiers[classifier]]
                )
        ...
        if (
            len(log) > 0
            and len(log[0]) > 0
            and "concept:name" in log[0][0]
            and "lifecycle:transition" in log[0][0]
        ):
            classifier_attr_key = "@@classifier"
            for trace in log:
                for event in trace:
...
def insert_trace_classifier_attribute(log, classifier):
    ...
    if classifier is not None:
        ...
        for trace in log:
            classifier_value = "+".join(
                [trace.attributes[x] for x in log.classifiers[classifier]]
            )
