...
class TestVisualizationDistinction:
    ...
    def test_choice_graph_visualization(self):
        """Test that choice graph visualization works without errors."""
        ...
        # Generate visualization - should work without errors
        viz = visualizer.apply(cg)

        # Check that visualization was generated
        assert viz is not None
        assert len(str(viz)) > 0
...
class TestVariantComparison:
    ...
    def test_maximal_vs_clustering(self):
        ...
        maximal_result = maximal_miner.apply(log)

        # Test CLUSTERING variant
        clustering_miner = InductiveMinerChoiceGraphClustering()
        clustering_result = clustering_miner.apply(log)
